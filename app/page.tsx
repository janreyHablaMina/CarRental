  ...
  return <main ref={mainRef} className="site-shell">
    <header className={`topbar ${progress > 0.015 ? "topbar-scrolled" : ""}`}>
      ...
      <nav className="desktop-nav" aria-label="Primary navigation">
        ...
        <Link href="/admin" className="admin-nav-link">Admin Portal</Link>
      </nav>
      <button className="nav-cta desktop-cta" onClick={() => scrollTo("#booking")}>Book now <ArrowRight size={16} /></button>
      <button className="menu-button" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu">{menuOpen ? <X /> : <Menu />}</button>
      {menuOpen && <nav className="mobile-nav"><button onClick={() => scrollTo("#vehicles")}>Vehicles</button><button onClick={() => scrollTo("#how")}>How It Works</button><button onClick={() => scrollTo("#why")}>About</button><button onClick={() => scrollTo("#booking")}>Book Now</button><Link href="/admin" className="admin-mobile-link">Admin Portal</Link></nav>}
    </header>
